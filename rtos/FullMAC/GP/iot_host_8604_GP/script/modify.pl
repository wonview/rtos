...
open(inifile,"./script/wifi.ini")or die " ./script/wifi.ini is not exist\n";;
...
my $set_mac = 0;
my $src_path  = './host/core/host_apis.c';
my $des_path  = './host/core/host_apis_backup.c';
...
#config.h
my $define_str='CONFIG_CHIP_ID';
my $config_src_path  = './host/include/priv/host_config.h';
my $config_des_path  = './host/include/priv/host_config_bak.h';
...
while($sline = <src>)
{
   ...
   else
   {
      ...
      my $soc = substr $sline,length("#define $define_str"),length($sline);
      print "soc=$soc\n";
...
system("perl ./script/ver_info.pl ./host/version.c" );
