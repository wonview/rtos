...
void SSID_Handler(char *pcInsert)
{
    ...
    info = OS_MemAlloc(sizeof(Ap_sta_status));
...
void Encrypt_Handler(char *pcInsert)
{
    ...
    if ((SSV6XXX_HWM_STA == info->operate ) && (info->status == true))
    {
        STRCPY(pcInsert, info->u.station.proto&WPA_PROTO_WPA?"WPA":
            (info->u.station.proto&WPA_PROTO_RSN?"WPA2":"OPEN"));
    }
    else if ((SSV6XXX_HWM_AP == info->operate ) && (info->status == true))
    {
    ...
    if ((SSV6XXX_HWM_SCONFIG == info->operate ) && (info->status == true))
    {
        STRCPY(pcInsert, info->u.station.proto&WPA_PROTO_WPA?"WPA":
            (info->u.station.proto&WPA_PROTO_RSN?"WPA2":"OPEN"));
    }
...
const char* WIFI_CGI_Handler(int iIndex, int iNumParams, char *pcParam[], char *pcValue[])
{
    ...
    wifi_sec_type encrypt_mode;
    char password[128];

    LOG_PRINTF("WIFI_CGI_Handler\r\n");
    OS_MemSET((void * )&ap_cfg, 0, sizeof(ap_cfg));
    OS_MemSET((void * )&sta_cfg, 0, sizeof(sta_cfg));
    ...
    if (iIndex != -1)
    {
        ...
        {
            STRCPY((void *)ssid, pcValue[iIndex]);
        }
    }
    else
    {
    ...
    if (iIndex != -1)
    {
        ...
        if (mode == SSV6XXX_HWM_AP)
        {
            ...
            else if (STRCMP(pcValue[iIndex],"wep40") == 0)
            {
                ap_cfg.status = TRUE;
                ap_cfg.security =   SSV6XXX_SEC_WEP_40;
            }
            else if (STRCMP(pcValue[iIndex],"wep104") == 0)
            {
                ap_cfg.status = TRUE;
        ...
        else if (mode == SSV6XXX_HWM_STA)
        {
            if (STRCMP(pcValue[iIndex],"open") == 0)
            {
                encrypt_mode = WIFI_SEC_NONE;
            }
            else if (STRCMP(pcValue[iIndex],"wep") == 0)
            {
                encrypt_mode = WIFI_SEC_WEP;
            }
            else if (STRCMP(pcValue[iIndex],"wpa") == 0)
            {
                encrypt_mode = WIFI_SEC_WPA_PSK;
            }
            else if (STRCMP(pcValue[iIndex],"wpa2") == 0)
            {
                encrypt_mode = WIFI_SEC_WPA2_PSK;
            }
            else
            {
                return "/run_status.shtml";
    ...
    if (iIndex != -1)
    {
        ...
        {
            STRCPY((void *)password, pcValue[iIndex]);
        }
        else
        {
            return "/run_status.shtml";
    ...
    else if (mode == SSV6XXX_HWM_STA)
    {
        ...
        if (STRLEN(ssid) != 0)
        {
            netmgr_wifi_join_async(ssid, encrypt_mode, password);
        }
    }
    #endif
    return "/run_status.shtml";
