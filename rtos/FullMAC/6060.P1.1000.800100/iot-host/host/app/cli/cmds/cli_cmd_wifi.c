...
void _scan_result_handler(void *data)
{
    ...
    LOG_PRINTF("SSID: %s \t", scan_res->apInfo[scan_res->index].ssid.ssid);
    ...
    //LOG_PRINTF("rpci=%d, snr=%d\r\n",scan_res->apInfo[scan_res->index].rxphypad.rpci,scan_res->apInfo[scan_res->index].rxphypad.snr);
    LOG_PRINTF("\r\n");
...
void _cmd_wifi_join (s32 argc, char *argv[])
{
    ...
    else if (strcmp(argv[1], "wep") == 0)
    {
        ret = netmgr_wifi_join_async(argv[0], WIFI_SEC_WEP, (argc == 3) ? argv[2]: NULL);
    }
...
void _cmd_wifi_list(s32 argc, char *argv[])
{
	...
	for (i=0; i<NUM_AP_INFO; i++)
    {
        if(gdeviceInfo->ap_list[i].channel_id!= 0)
		{
            ...
            //LOG_PRINTF("rpci=%d, snr=%d\r\n",gdeviceInfo->ap_list[i].rxphypad.rpci,gdeviceInfo->ap_list[i].rxphypad.snr);
            LOG_PRINTF("\r\n");
		}
	}
